...
def ReCalculateNPQ(Corpus, Cut, LengthPenaltyConstant, PhraseFrequency, OldNPQ) :
	...
	LenFrequency = collections.defaultdict(int)
	for Phrase in PhraseFrequency :
		LenFrequency[len(Phrase)] += 1
	Pos = LenCorpus
	while Pos :
		NextPos = Cut[Pos]
	...
	for Phrase in NewNPQ :
		NewNPQ[Phrase] = NewNPQ[Phrase] + Occurence[Phrase]/LenFrequency[len(Phrase)]
	Sum = 0.0
	for Phrase in NewNPQ :
